        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
                        <div class="col-sm-4">
                            <button type="button" class="btn btn-info mb-2" data-bs-toggle="modal" data-bs-target="#add"><i class="mdi mdi-plus-circle me-2"></i> Add Admin</button>
                        </div>
                    </div>
                            ...
                            @foreach ($admins as $admin)
                                <tr>
                                    ...
                                    <td class="table-action" data-id="{{$admin->id}}">
                                        <a href="javascript: void(0);" class="action-icon" data-bs-toggle="modal" data-bs-target="#edit"  id="editRow" data-id="{{$admin->id}}"> <i class="mdi mdi-pencil"></i></a>
                                        <a href="javascript: void(0);" class="action-icon"  id="deleteRow" data-id="{{$admin->id}}"> <i class="mdi mdi-delete"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
...
<script>
    ...
    $('body').on('submit', '#addNewDataForm', function(e) {
        ...
        axios.post("{{ route('admins.store') }}", {
                ...
                window.location.href = "{{ route('admins.store') }}"
            })
    ...
    $('body').on('click', '#editRow', function() {
        let id = $(this).data('id');
        let edit = '/' + window.location.pathname.split('/')[1] + '/dashboard/admins/' + id + '/edit';
        axios.get(edit)
        .then(function(res) {
            // console.log(res);
    ...
    $('body').on('submit', '#editDataForm', function(e) {
        e.preventDefault()
        let id = $('#e_id').val()
        axios.put('/' + window.location.pathname.split('/')[1] + '/dashboard/admins/' + id, {
                role_id: $('#e_role').val(),
